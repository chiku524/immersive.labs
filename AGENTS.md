...
- Recurring ops guidance includes keeping the tunnel API hostname DNS-only where recommended, monitoring cloudflared and app logs during 502s, and tuning Ollama via model size, RAM or swap, mock mode, or STUDIO_OLLAMA_READ_TIMEOUT_S with the tradeoff that long timeouts occupy the queue worker; operators may raise STUDIO_OLLAMA_READ_TIMEOUT_S into the thousands of seconds for long full-job generations when they accept longer worker occupancy.
