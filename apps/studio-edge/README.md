...
2. **Python:** raise **`STUDIO_OLLAMA_READ_TIMEOUT_S`** only if reads are legitimately long; default base when unset is **3000s**, with a second read attempt at ~**1.5×** (capped **7200s**/attempt). Ollama returns when the model finishes — long caps do not add latency on success. Prefer **faster models / more RAM** if generations are slow. If logs show **~900s**, check GCE metadata / Docker env for a pinned **900**.
