...
2. **Python:** the worker uses **Ollama streaming by default** (better read-timeout behavior than blocking mode). From **immersive-studio 0.1.9**, the default base read timeout when unset is **600s** (clamp **15–3600**; second attempt ~**1.5×**, capped **3600s**/attempt), with **connect** timeout default **8s** and **preflight** (`GET /api/tags`) + **model verify** on by default — unreachable or misconfigured Ollama should fail much faster than older **3000s/14400s** defaults. Raise **`STUDIO_OLLAMA_READ_TIMEOUT_S`** only if generations legitimately need more wall time, or set **`STUDIO_OLLAMA_DISABLED=1`** for mock-only hosts. Prefer **faster models / more RAM** if generations are slow. If logs still show **~900s** reads, check GCE metadata / Docker env for a pinned **`STUDIO_OLLAMA_READ_TIMEOUT_S=900`**.
