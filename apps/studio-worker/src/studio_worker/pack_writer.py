...
def _unity_collider_to_bevy(collider: Any) -> str:
    mapping = {"box": "box", "capsule": "capsule", "mesh_convex": "convex", "none": "none"}
    if isinstance(collider, str):
        return mapping.get(collider.strip().lower(), "box")
    return "box"
...
def _bevy_import_notes(
    *,
    ...
    lines = [
        ...
        "",
        f"- **Job ID:** `{job_id}`",
        "- **Target engine:** Bevy **0.19** (current stable) with Immersive Studio helpers (`packages/studio-bevy`).",
        "",
        "## Pack contents",
        "",
        "- `manifest.json` — full job manifest (specs + toolchain metadata).",
        "- `Models/` — `.glb` meshes per asset (Tripo text-to-3D or Blender placeholder fallback).",
        "- `Textures/` — PBR PNGs (`{variant}_{slot}_albedo|normal|orm.png`).",
        "- `Bevy/pack_registry.rs` — auto-generated registry snippet for `immersive_studio_bevy::AssetRegistry`.",
        "",
        "## Assets in this job",
        "",
    ]
    for a in assets:
        ...
        bevy = a.get("bevy") or {}
        unity = a.get("unity") or {}
        sub = bevy.get("import_subfolder") or "models"
        collider = bevy.get("collider") or _unity_collider_to_bevy(unity.get("collider"))
        height = a.get("target_height_m", "?")
        lines.append(
            f"- **{aid}** — style `{st}`, copy under `assets/{sub}/{aid}/`, "
            f"collider `{collider}`, target height `{height}` m."
        )
    lines.extend(
        [
            ...
            "## Import checklist",
            "",
            "1. Copy `Models/<asset_id>/` into your Bevy `assets/` folder at `assets/<bevy.import_subfolder>/<asset_id>/`.",
            "2. Copy sidecar textures from `Textures/<asset_id>/` into the same folder (when ComfyUI or bind step ran).",
            "3. Add `immersive-studio-bevy` from `packages/studio-bevy` (path dependency) with `bevy = \"0.19\"`.",
            "4. Copy `Bevy/pack_registry.rs` into your crate (or include! it) and call `register_all(&mut registry)` at startup.",
            "5. Spawn with `immersive_studio_bevy::spawn_prop(commands, &asset_server, &registry, asset_id)` "
            "(uses `WorldAssetRoot` + `#Scene0` — Bevy 0.19 glTF API).",
            "6. Sidecar PBR: `apply_orm_sidecars` maps albedo/normal/ORM onto `StandardMaterial` "
            "(ORM R=AO, G=roughness, B=metallic — matches glTF metallicRoughness packing).",
            "7. Collision per `bevy.collider`: add a physics collider (`box`/`capsule`/`convex` from "
            "`{asset_id}_collider.glb` when present) via your physics plugin; `none` → visual-only.",
            "8. Units: 1 unit = 1 meter (same as the studio Unity convention).",
            "",
            "Deep reference: `docs/studio/bevy-import-conventions.md` in the immersive.labs repo.",
            "",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _write_bevy_pack_registry(
    output_dir: Path,
    *,
    job_id: str,
    assets: list[dict[str, Any]],
) -> None:
    bevy_dir = output_dir / "Bevy"
    bevy_dir.mkdir(parents=True, exist_ok=True)
    lines = [
        "// Auto-generated by Immersive Studio — merge into your Bevy project and call at startup.",
        "use immersive_studio_bevy::AssetRegistry;",
        "",
        f'pub const JOB_ID: &str = "{job_id}";',
        "",
        "pub fn register_all(registry: &mut AssetRegistry) {",
    ]
    if not assets:
        lines.append("    let _ = registry;")
    else:
        for a in assets:
            aid = a["asset_id"]
            bevy = a.get("bevy") or {}
            sub = str(bevy.get("import_subfolder") or "models").strip().strip("/")
            model = f"{sub}/{aid}/{aid}.glb"
            texture_root = f"{sub}/{aid}"
            height = a.get("target_height_m")
            height_arg = (
                f"Some({float(height):.6g})" if isinstance(height, (int, float)) else "None"
            )
            lines.extend(
                [
                    "    registry.register(",
    ...
    lines.append("}")
    (bevy_dir / "pack_registry.rs").write_text("\n".join(lines) + "\n", encoding="utf-8")


def append_bevy_diagnostics_notes(output_dir: Path, notes: list[str]) -> None:
    """Append pack diagnostics to BevyImportNotes.md without replacing the import guide."""
    if not notes:
        return
    path = output_dir / "BevyImportNotes.md"
    if path.is_file():
        existing = path.read_text(encoding="utf-8").rstrip()
    else:
        existing = "# Bevy import notes\n"
    block = "\n## Pack diagnostics\n\n" + "\n".join(f"- {line}" for line in notes)
    path.write_text(existing + block + "\n", encoding="utf-8")
...
def _pack_readme(*, job_id: str, engine_target: str) -> str:
    ...
    notes_by_target = {
        ...
        "bevy": "BevyImportNotes.md (copy Models + Bevy/pack_registry.rs; packages/studio-bevy helpers)",
    ...
    return (
        ...
        "Meshes: Models/<asset_id>/*.glb — same GLB for Unity, Unreal, Godot, and Bevy "
        "(Tripo or Blender fallback)\n"
        f"Primary import guide: {primary_notes}\n"
        "Also included: UnityImportNotes.md, UnrealImportNotes.md, GodotImportNotes.md, "
        "and BevyImportNotes.md\n"
    )
...
def write_pack(
    ...
    *,
    ...
    (output_dir / "BevyImportNotes.md").write_text(
        _bevy_import_notes(
            job_id=manifest["job_id"],
            assets=manifest["assets"],
        ),
    ...
    for a in manifest["assets"]:
        ...
        readme.write_text(
            "Engine-agnostic GLB (Unity + Unreal + Godot + Bevy): Tripo text_to_model or Blender placeholder fallback.\n"
