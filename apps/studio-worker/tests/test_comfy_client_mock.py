...
@respx.mock
def test_run_txt2image_workflow_retries_transient_history_502(monkeypatch: pytest.MonkeyPatch) -> None:
    """Brief /history 502 while Comfy renders should not abort the whole texture step."""
    base = "http://comfy.retry502"
    monkeypatch.setenv("STUDIO_COMFY_URL", base)

    respx.post(f"{base}/prompt").mock(
        return_value=httpx.Response(200, json={"prompt_id": "pid-502"})
    )
    respx.get(f"{base}/history/pid-502").mock(
        side_effect=[
