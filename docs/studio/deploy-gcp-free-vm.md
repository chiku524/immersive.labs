...
4. **Mock mode** in `/studio` skips Ollama for that request. **`STUDIO_OLLAMA_DISABLED=1`** on the container forces mock specs for **all** jobs and generate-spec calls when you do not run Ollama on the VM.
