...
The **`/studio`** page imports **`StudioDashboardPayload`**, **`StudioUsageInfo`**, **`StudioBillingStatus`**, **`StudioJobSummary`**, and **`StudioComfyStatusPayload`** from this package so UI types stay aligned with **`GET /api/studio/dashboard`** and related worker routes.
