...
- `engine_target` in `manifest.json` is set by the worker (`unity`, `unreal`, `godot`, or `bevy`) when you choose the import target in `/studio` or pass `--engine-target` on the CLI.
