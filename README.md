...
Use **mock mode** in the UI if Ollama is not running, or set **`STUDIO_OLLAMA_DISABLED=1`** on the worker so all specs use the mock path without Ollama. Enable **Generate albedo textures** when [ComfyUI](https://github.com/comfyanonymous/ComfyUI) is up and `STUDIO_COMFY_CHECKPOINT` matches a local checkpoint. See [apps/studio-worker/README.md](./apps/studio-worker/README.md) and [packages/studio-unity/README.md](./packages/studio-unity/README.md).
